...
def _write_details_parquet(path, details: list[DetailsLogger.Detail]) -> None:
    """Write `details` as a single-row-group parquet file, matching `write_task_batch`'s schema."""
    path.parent.mkdir(parents=True, exist_ok=True)
    rows = EvaluationTracker._details_rows(details)
...
def test_scoreboard_callback_selects_twenty_samples_per_outcome(tmp_path):
    details = []
    ...
    path = tmp_path / "details_task_2026.parquet"
    _write_details_parquet(path, details)
...
def test_scoreboard_splits_rollouts_and_scores_each_one(tmp_path):
    ...
    rollouts = [rollout for bucket in accumulator.selected.values() for rollout in bucket]
    rollouts.sort(key=lambda rollout: rollout.repeat_id)
...
def test_scoreboard_waits_for_all_internal_leaves_and_publishes_only_selector():
    ...
    callback._selector_completed = {}
    publications = []
    callback._publish_selector = lambda selector, tasks: publications.append((selector, tasks))
    ...
    assert callback._selector_completed == {}
...
def test_scoreboard_publication_keeps_only_evaluation_facts(tmp_path, monkeypatch):
    ...
    details_path = tmp_path / "details_gsm8k_2026.parquet"
    _write_details_parquet(details_path, [detail])
    pipeline = SimpleNamespace(
    ...
    tracker = SimpleNamespace(
        ...
        task_details_path=lambda task_name: details_path,
    )
